...
export class FeedFetcher {
  ...
  async fetchFromUrl(feedUrl: string): Promise<{ products: Product[], metadata: FeedMetadata }> {
    ...
    try {
      ...
      if (contentLength && parseInt(contentLength) > this.MAX_FEED_SIZE) {
        throw new Error(`Feed too large: ${parseInt(contentLength) / 1024 / 1024}MB (max: ${this.MAX_FEED_SIZE / 1024 / 1024}MB)`);
      }
      ...
      } else if (format === 'json' || contentType.includes('json') || content.trim().startsWith('{') || content.trim().startsWith('[')) {
        const result = await this.parseJsonFeed(content);
        products = result.products;
        metadata = { ...metadata, ...result.metadata, format: 'json' };
      } else if (format === 'csv' || format === 'tsv' || contentType.includes('csv') || contentType.includes('tab-separated')) {
      ...
      throw new Error(`Failed to fetch/parse feed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async parseXmlFeed(content: string): Promise<{ products: any[], metadata: Partial<FeedMetadata> }> {
    ...
    while ((match = entryRegex.exec(content)) !== null) {
      ...
      const extractField = (fieldName: string, xmlTag: string = fieldName) => {
        const regex = new RegExp(`<(?:g:)?${xmlTag}><!\\[CDATA\\[([^\\]]+)\\]\\]><\\/(?:g:)?${xmlTag}>|<(?:g:)?${xmlTag}>([^<]+)<\\/(?:g:)?${xmlTag}>`);
      ...
      const additionalImages = entry.match(/<(?:g:)?additional_image_link>([^<]+)<\/(?:g:)?additional_image_link>/g);
      if (additionalImages) {
  ...
  private async parseJsonFeed(content: string): Promise<{ products: any[], metadata: Partial<FeedMetadata> }> {
    try {
  ...
  private async parseCsvFeed(content: string, delimiter: string = ','): Promise<{ products: any[], metadata: Partial<FeedMetadata> }> {
  ...
  private async autoDetectAndParse(content: string): Promise<{ products: any[], metadata: Partial<FeedMetadata> }> {
  ...
  private normalizeProducts(products: any[]): Product[] {
    return products.map(product => {
      ...
      if (!product.id) {
        product.id = product.sku || product.item_id || product.product_id || Math.random().toString(36).substr(2, 9);
      }
