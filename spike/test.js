...
function runPerformanceTest(nodeCount) {
  ...
  function render() {
    ...
    if (simulation.alpha() > simulation.alphaMin()) {
      ...
      const avgRenderTime = (renderTimes.reduce((a, b) => a + b, 0) / renderTimes.length).toFixed(
        2
      );
...
function generateReport() {
  ...
  const memoryUnder200 = testResults.every((r) => r.memoryUsage === 'N/A' || r.memoryUsage < 200);

  const recommendation =
    can1000At60 && can3000At30 && memoryUnder200
