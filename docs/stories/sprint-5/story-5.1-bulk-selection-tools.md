...
1. **Lasso selection tool**
   ...
   class LassoSelectionTool {
     ...
     private handleMouseDown(event: MouseEvent) {
       ...
       this.lassoPath = lassoLayer.append('path')
         .attr('class', 'lasso')
...
2. **Multi-select manager**
   ...
   class SelectionManager {
     ...
     selectByAttribute(attribute: string, value: any) {
       const matchingNodes = this.visualization.getNodesWhere(
     ...
     private updateVisualization() {
       ...
       this.visualization.updateNodeStyles((node) => ({
         ...
         opacity: this.selectedNodes.size === 0 ? 1 : 
                  this.selectedNodes.has(node.id) ? 1 : 0.3
       }));
