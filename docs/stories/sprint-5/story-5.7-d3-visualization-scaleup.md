...
1. **Performance-optimized renderer**
   ...
   export class OptimizedD3Renderer {
     ...
     constructor(
       ...
       optimizations: Partial<RenderOptimizations> = {}
     ) {
       this.canvas = canvas;
     ...
     initializeSimulation(nodes: any[], links: any[]) {
       ...
       this.simulation = d3.forceSimulation(nodes)
         ...
         .force('charge', d3.forceManyBody()
           .strength(-50)
           .distanceMax(200) // Limit charge force range
         )
         ...
         .force('collision', d3.forceCollide()
           .radius(10)
           .iterations(1) // Reduce collision detection iterations
         )
     ...
     private startRenderLoop() {
       ...
       const render = () => {
         ...
         if (deltaTime > 1000 / 30) { // Below 30 FPS
           this.reduceQuality();
         } else if (deltaTime < 1000 / 60) { // Above 60 FPS
     ...
     private getVisibleNodes(nodes: any[]): any[] {
       ...
       this.quadtree.visit((node, x1, y1, x2, y2) => {
         if (node.data) {
           const d = node.data;
           if (
             d.x >= worldBounds.x1 && d.x <= worldBounds.x2 &&
             d.y >= worldBounds.y1 && d.y <= worldBounds.y2
           ) {
             visible.push(d);
           }
         }
         ...
         return x1 > worldBounds.x2 || x2 < worldBounds.x1 ||
                y1 > worldBounds.y2 || y2 < worldBounds.y1;
       });
     ...
     private getVisibleLinks(links: any[], visibleNodes: any[]): any[] {
       ...
       return links.filter(link => 
         nodeSet.has(link.source.id) || nodeSet.has(link.target.id)
       );
     }
     ...
     getNodeAtPosition(x: number, y: number): any {
       ...
       if (this.quadtree) {
         ...
         this.quadtree.visit((node, x1, y1, x2, y2) => {
           ...
           // Prune search
           return x1 > worldX + 20 || x2 < worldX - 20 ||
                  y1 > worldY + 20 || y2 < worldY - 20;
         });
