...
2. **Recovery system**
   ...
   class RecoveryManager {
     ...
     private async restoreDatabase(dbBackup: DatabaseBackup) {
       ...
       await execAsync(`psql ${process.env.DATABASE_URL} -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"`);
...
3. **Disaster recovery procedures**
   ...
   class DisasterRecoveryManager {
     ...
     private createDataCorruptionRecoveryPlan(
       assessment: DamageAssessment
     ): RecoveryPlan {
       return {
         ...
         steps: [
           ...
           },
           {
             ...
             critical: true,
             execute: async () => {
               const backup = await this.findLastGoodBackup(
                 assessment.corruptionDetectedAt
               );
               return { backup };
             }
           },
           {
             ...
             execute: async (context) => {
               await this.recoveryManager.restoreFromBackup(
                 context.backup.id,
