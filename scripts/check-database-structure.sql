...
SELECT 
    CASE 
        WHEN tables_count = 2 THEN '✅'
        ELSE '❌'
    END || ' Tables: ' || tables_count || '/2' as tables_status,
    CASE 
        WHEN new_columns_count = 5 THEN '✅'
        ELSE '❌'
    END || ' New Columns: ' || new_columns_count || '/5' as columns_status,
    CASE 
        WHEN indexes_count >= 10 THEN '✅'
        ELSE '⚠️'
    END || ' Indexes: ' || indexes_count || '/10+' as indexes_status,
    CASE 
        WHEN tables_count = 2 AND new_columns_count = 5 THEN '✅ MIGRATION SUCCESSFUL'
        WHEN tables_count > 0 OR new_columns_count > 0 THEN '⚠️ PARTIALLY APPLIED'
